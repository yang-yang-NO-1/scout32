...
int endpoint = 220  ; // Multiplier for servo endpoints. Should be around 60 on modded servos, around 220 for TowerPro 360.
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    else if(!strcmp(variable, "trim")) 
    {
      ...
      if (val==1) {
        ...
        ledcWrite(3,(neutral+trim+speed));  // GPIO pin 12 - Left Servo adds speed to neutral value
        ledcWrite(4,(neutral+trim-speed));  // GPIO pin 13 - Right Servo subtracts speed from neutral value
      }
      else if (val==2) {
        Serial.println("Backward");  
        actstate = rev; // Set state to modify left & right behavior while moving.      
        ledcWrite(3,(neutral-trim-speed));  // GPIO pin 12 Left Servo
        ledcWrite(4,(neutral-trim+speed));  // GPIO pin 13 Right Servo        
      }
      else if (val==3) {
        Serial.println("TurnLeft");
        if      (actstate == fwd) { ledcWrite(3,neutral); ledcWrite(4,(neutral-speed)); }
        else if (actstate == rev) { ledcWrite(3,neutral); ledcWrite(4,(neutral+speed)); }
        else                      { ledcWrite(3,(neutral-speed)); ledcWrite(4,(neutral-speed)); }          
      }
      else if (val==4) {
        Serial.println("TurnRight");
        if      (actstate == fwd) { ledcWrite(3,(neutral+speed)); ledcWrite(4,neutral); }
        else if (actstate == rev) { ledcWrite(3,(neutral-speed)); ledcWrite(4,neutral); }
        else                      { ledcWrite(3,(neutral+speed)); ledcWrite(4,(neutral+speed)); }        
      }
      else {
        Serial.println("Stop");
        actstate = stp;
        ledcWrite(3,neutral); // Send no pulse to disable servo (no drift).
        ledcWrite(4,neutral); // Send no pulse to disable servo (no drift).       
      } 
    }        
    else 
    { 
